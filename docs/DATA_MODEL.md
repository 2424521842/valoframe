...
三张 `ad_*` 表只保存广告方素材元信息与本地计数，不含任何玩家、对局或文件字段；广告接口与落地域名信任边界来自发行构建的维护者配置，不写入 SQLite，也不再属于前端用户偏好（localStorage）。
