...
| 正式安装与发布 | 🟡 发布中 | 稳定 updater 工程、真实 Tauri 密钥/仓库配置与加密离线备份已就绪；正在发布 v0.2.1，随后仍需完成 v0.2.1→v0.2.2 首次 OTA。Authenticode 与严格 policy 为未来可选加固 |
...
- [x] 本地生成带强密码的 updater 密钥；私钥/密码已配置为仓库 Secrets，公钥已配置为 `VALOFRAME_UPDATER_PUBLIC_KEY` Variable，并验证加密离线备份可读取；仓库不包含真实私钥。
