...
| SQLite schema 与迁移 | ✅ 已完成 | schema v13；评分来源、分页/缩略图索引、回收身份快照、永久删除 outbox、未来版本拒绝、在线备份与原子升级 |
...
| 扫描互斥、进度与取消 | ✅ 已完成 | 后台任务、job ID、already-running、事件、取消和终态恢复已接入 |
| 四路元数据采集 | ✅ 已完成 | WonderfulDb、导出 JSON、highlight.log、LevelDB 容错合并 |
| 稳定账号身份与来源 DTO | ✅ 已完成 | 稳定 ID 分组；`list_sources` 返回路径、状态、错误、数量和扫描时间 |
...
- [x] 使用 `PRAGMA user_version` 管理 schema v13。
