...
应用启动先完成单实例交接、数据库迁移、索引加载，以及异常中断扫描/永久删除意图的恢复。全局“启动时自动扫描”默认关闭；用户开启后，前端在下次启动读取偏好并请求后端对所有加入自动同步的来源执行一次非阻塞增量同步，10 分钟内重启不会重复扫描。用户始终可以手动同步单一来源或全部已加入来源。后端按扫描模式进入以下流程：
