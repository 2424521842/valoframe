...
| `ads.rs` | 构建时受信任广告入口/落地域名配置、广告清单拉取与校验（HTTPS 强制、字段长度、RFC 3339 投放期与 ID 字符白名单）、素材图魔数校验与本地缓存、落地页域名允许列表校验与 `click_id` 生成 |
| `commands/ads.rs` | 不接受前端接口或域名参数的广告素材列表/清单刷新、曝光计数、点击记录与经 `ShellExecuteW` 打开系统浏览器 |
...
| 广告位（维护后台控制） | `list_ad_creatives`（受信任配置有效时读本地缓存，不联网）、`refresh_ad_creatives`（从构建内置固定入口拉取并校验清单、缓存素材图；失败时清空活动）、`record_ad_impression`、`record_ad_click`（落库 `click_id` 后按构建内置域名允许列表打开系统浏览器，返回 `click_id`） |
...
- 问题反馈是唯一由用户主动发起的上行数据路径：只有用户在反馈弹窗中主动提交时才会发起请求，目标只能是随命令传入的 HTTPS 接口（本机自测允许 localhost HTTP，其余 http 一律拒绝）；当前接口配置入口不在 UI 中、endpoint 恒为空，因此实际只走“保存 zip 文件”路径，上传代码保持同样约束待后台就绪后启用。广告链路只下载受信任固定入口的清单与素材，点击后由系统浏览器访问落地页，不上传应用内用户数据。诊断数据按脱敏白名单组装（不含 OpenID/PUUID、备注、标签、绝对路径），完整视频最大 1 GiB、描述 2000 字、上传超时 15 分钟；保存/放弃命令只接受应用反馈缓存目录内的 zip 路径。
- 广告位不引入任何第三方 SDK：CSP 不放开、capability 不新增。素材入口和落地域名信任边界由发行构建的 `VALOFRAME_AD_MANIFEST_ENDPOINT` / `VALOFRAME_AD_ALLOWED_HOSTS` 固定，前端命令不接受接口或允许列表参数，v0.2.4 的 `adsEnabled`、`adManifestEndpoint`、`adAllowedHosts` 本机偏好会被忽略并清理。广告清单仍是不可信输入，由 Rust 侧校验 HTTPS、字段长度、RFC 3339 投放期、素材 ID 字符白名单（仅 `A-Za-z0-9-_`，杜绝路径穿越）与落地页模板占位符后才入库；单条非法素材被丢弃，空活动或刷新失败会清空/隐藏广告而不是展示旧缓存。素材图按魔数而非 `Content-Type` 判定 PNG/JPEG，限 512 KiB，缓存 basename 经 `resolve_cached_image` 校验不得逸出缓存目录，再经 `clip-media://ad/<id>` 投递，因此前端始终零外域请求。点击时后端从库内模板重建 URL（不接受前端传入的 URL），校验协议为 https 且域名命中构建内置允许列表（精确匹配或真子域，不做后缀匹配）；随后先落 `ad_click_log` 再调用 `ShellExecuteW`，确保到达广告方的点击都可对账。传出数据仅 `sub_id` 与一次性 `click_id`，应用内不收集任何个人信息，表单一律在广告方落地页完成。详见 `docs/AD_INTEGRATION_SPEC.md`。
